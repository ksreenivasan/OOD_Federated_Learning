...
def train_nd_constain(epoch, net, ori_net, mu):
    print('\nEpoch: %d' % epoch)
    net.train()
    train_loss = 0
    correct = 0
    total = 0
    global_weight_collector = [param.data for param in ori_net.parameters()]
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        #########################we implement norm diff constraint Here##############################
        fed_prox_reg = 0.0
...
# i) load normally trained checkpoints and start grayscale training
#net.load_state_dict(torch.load('./checkpoint/ckpt_vanilla_150.pth', map_location=device)['net'])
saved_dict = torch.load('./checkpoint/ckpt_vanilla_50.pth', map_location=device)['net']
manual_loader(saved_dict, net, device=device)
manual_loader(saved_dict, ori_net, device=device)


# after loading the checkpoint, let's validate the model
test(0, False, net=net)
test(0, True, net=net)

calc_norm_diff(gs_model=net,
               vanilla_model=ori_net,
               epoch=0)
...
for epoch in range(start_epoch, start_epoch+100):
    ...
    #     train(epoch)
    #train(epoch, net=net)
    train_nd_constain(epoch, net=net, ori_net=ori_net, mu=0.1)

    calc_norm_diff(gs_model=net,
                   vanilla_model=ori_net,
                   epoch=epoch)

    test(epoch, False, net=net) # test for vanilla testset
    test(epoch, True, net=net)  # test for grayscale testset (only contains airplane)
    ...
    torch.save(net.state_dict(), './ckpt_retrain_%d.pth' % epoch)
